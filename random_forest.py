...
def build_smote_pipeline():
    ...
    # even though the forest itself is scale-invariant.
    return ImbPipeline([
...
def get_permutation_importance(model, X_test, y_test):
    """Calculates permutation importance, measured as the drop in F1 when a
    single feature's values are shuffled. F1 is used instead of the default
    accuracy since that's the metric these models are tuned on, and it stays
    sensitive to minority-class predictions under class imbalance."""
